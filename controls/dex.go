...
func TokenChainPriceHandler3(c *gin.Context) {
	dataProc := func(tokenCode string) (interface{}, error) {
...
where (symbol0 = ?
   ...
   and vol_usd>?
   and symbol=?
;`, symbol, symbol,50000,symbol).Find( &pairs).Error
		if err != nil {
			return nil, err
		}
		if len(pairs) == 0 {
...
select *
from (
         select avg(prices.price) avg, max(prices.price) high, min(prices.price) low
         from uni_prices prices
         where prices.pair_id = ?
           and prices.symbol = ?
           and prices.block_time>unix_timestamp() - 240 * 60
     ) a
where a.high is not null`, pair.Id, pair.Symbol).First(vp).Error
			if err != nil {
				vp.Avg = vp.Last
				vp.Low = vp.Last
				vp.High = vp.Last
...
func IsSignAble(code string, price float64) (signAble bool, msg string) {
	...
	if signAble && isFtx(code) {
		...
		if !IsDisableCheckFtxDataNewCheck && !services.IsFtxDataNew(coinType,delay){
			signAble = false
...
var safePrice = map[string]*mm{
	...
	"0x5190144c70f024bbccf9b41690e4ce3ccac31a68": &mm{95000, 240000},
	"0x66094a0624a4e8a8b9a7eff8dc0982706015340d": &mm{10000, 38000},
	"0x247913d11957f3561d4a14166ec478c3c70a9297": &mm{2800, 8000},
	"0xb1c1504c6f2646cad9ed291158b694723d38c394": &mm{250, 800},
	"0xab9016557b3fe80335415d60d33cf2be4b9ba461": &mm{3800, 13680},
	"0x6b5ab672ac243193b006ea819a5eb08bcd518de7": &mm{200, 400},
	"0xc7b86cc68c2b49f2609e9b5e12f0aa7be775bf1d": &mm{3, 12},
